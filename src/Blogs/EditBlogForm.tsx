...
function EditBlogForm() {
    ...
    const fetchBlog = async () => {
        ...
        await axios(`http://localhost:8080/api/get/${params.BlogID}`)
        .then(res => {
            if (res.data.success) {
    ...
    const editBlog = async () => {
        ...
        await axios.patch(`http://localhost:8080/api/edit/${params.BlogID}`, 
        {
            Title: titleValue,
    ...
    return (
        <div className="main" style={{display: 'flex', justifyContent: 'center'}}>
            ...
            <Textarea
                ...
                variant="filled"
                size="md"
                onChange={(event: React.ChangeEvent<HTMLTextAreaElement>) => footerValueChange(event.target.value)}
                value={footerValue}
            />
            </Stack>
